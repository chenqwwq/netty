...
public final class NioEventLoop extends SingleThreadEventLoop {
    ...
    /**
     * 构造函数中通过{@link NioEventLoop#openSelector()}方法获取selector对象
     * 但此时并没有绑定任何的Channel
     */
    NioEventLoop(NioEventLoopGroup parent, Executor executor, SelectorProvider selectorProvider,
        ...
        // 这个Selector会根据环境类型选择不同的底层实现
        // 返回的SelectorTuple会带有一个被包装的Selector
    ...
    private SelectorTuple openSelector() {
        final Selector unwrappedSelector;
        try {
            // 通过SelectorProvider生成Selector对象
    ...
    /**
     * run方法就是EventLoop轮询的完整逻辑
     * <p>
     * 注意区分绑定的Channel，NioServerSocketChannel和NioSocketChannel处理的不同的IO事件
     */
    @Override
    protected void run() {
        int selectCnt = 0;
